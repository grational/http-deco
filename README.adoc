image::images/logo-2x.png[width=800]

## **Introduction**

`http-decò` is a library designed to make executing HTTP requests in JDK languages as easy as possible. It provides a concise and intuitive API for basic HTTP methods and advanced features such as automatic retries, request caching, and AWS authentication.

**Features**

## **Basic HTTP methods:**

```
* GET
* POST
* PUT
* DELETE
* HEAD
```

## **Facilitating HTTP methods:**

```asciidoc
* POST with JSON payload
* PUT with JSON payload
* POST of type x-www-form-urlencoded
```

## **Decorators:**

```asciidoc
* Automatic retry in case of failure
* Automatic request caching with file storage or Redis
* Multi-host redirection
* AWS v4 authentication signing
```

## **Additional features:**

```asciidoc
* Support for basic authentication directly from URL userinfo
* Native support for http_proxy, https_proxy, no_proxy environment variables
* Easy disabling of HTTPS checks for testing with self-signed certificates
* Easy disabling of exceptions for failed requests
* Straightforward support for adding headers to requests
* Straightforward support for adding cookies to requests
```

## **Installation**

The http-decò library is available through JitPack. Groovy is an exported
runtime dependency, so select the artifact matching the Groovy major used by
your application.

[cols="1,1,1,1,3", options="header"]
|===
| Version
| Branch
| Java
| Groovy
| Use it when

| `v4.8.0`
| `master`
| 17+
| 5.0.4
| The application uses the current Groovy line.

| `v4.8.0-g4`
| `groovy-4`
| 17+
| 4.0.30
| The application depends on Groovy 4.

| `v4.9.0-j8g3`
| `java-8-groovy-3`
| 8+
| 3.0.25
| The application must remain compatible with Java 8 and Groovy 3.
|===

For the Groovy 3 release with the credential methods documented below, add the
following dependency to your `build.gradle` file:

```groovy
repositories {
	maven { url = uri('https://jitpack.io') }
}

dependencies {
	implementation 'com.github.grational:http-deco:v4.9.0-j8g3'
}
```

Or for your `pom.xml` file:
```xml
<dependency>
	<groupId>com.github.grational</groupId>
	<artifactId>http-deco</artifactId>
	<version>v4.9.0-j8g3</version>
</dependency>
```

## **Basic authentication**

Pass literal credentials to `withBasicAuth()` or to the `username` and
`password` parameters of `StructuredURL`. Use the explicit encoded entry points
when the credentials have already been encoded:

```groovy
import it.grational.http.request.Get
import it.grational.url.StructuredURL
import it.grational.url.UserInfo

// These requests use the same password: pa@ss
new Get('https://example.com/api'.toURL())
	.withBasicAuth('user', 'pa@ss')
new Get('https://example.com/api'.toURL())
	.withEncodedBasicAuth('user', 'pa%40ss')

// The equivalent StructuredURL forms:
def literal = new StructuredURL (
	protocol: 'https',
	authority: 'example.com',
	path: '/api',
	username: 'user',
	password: 'pa@ss'
).toURL()

def encoded = new StructuredURL (
	protocol: 'https',
	authority: 'example.com',
	path: '/api',
	credentials: UserInfo.encoded('user', 'pa%40ss')
).toURL()

assert literal.toString() == encoded.toString()
```

The encoded entry points decode once using UTF-8 (Unicode Transformation Format,
8-bit) and form-encoding rules: `%20` or `+` means a space, and `%2B` means a
literal plus. Malformed percent escapes are rejected. Literal entry points
preserve text such as `%20` as part of the password.

`StructuredURL` also accepts `credentials: new UserInfo(username, password)` for
literal credentials. Combining `credentials` with either `username` or `password`
is rejected, including when those parameters are empty or null.

The constructor parameter is `credentials`; the earlier `userInfo` spelling is
rejected with a migration message. If you downloaded the initial
`v4.9.0-j8g3` build, refresh Gradle dependencies with `--refresh-dependencies`
after updating the parameter name.

## **Usage example**

```groovy
import it.grational.http.request.*
import it.grational.cache.*
import java.time.Duration

// Example of a GET request
def response = new Get("https://www.google.com").connect()

// Example of a POST request with JSON payload
def json = [name: "John Doe", age: 30]
response = new JsonPost (
	url: "https://api.example.com/users",
	map: [
		name: 'John Doe',
		age: 30
	]
).connect()

// Usage example with automatic retry (3)
def response = new Retry (
   new Get("https://www.polito.it")
)

// Usage example with file caching limited to 5 minutes
def response = new Cache (
	new Get("https://www.polito.it"),
	new CacheFile(),
	Duration.ofMinutes(5)
)
```
